...
<script>
	...
	function fn_show_next_goods(){
		var img_sticky=document.getElementById("imgSticky");
		var cur_goods_num=document.getElementById("curGoodsNum");
		var _h_goods_id=document.frm_sticky.h_goodsId;
		var _h_goods_fileName=document.frm_sticky.h_goodsFileName;
		...
		img_sticky.src=SERVER_URL+"?goodsId="+goods_id+"&goodsFileName="+fileName;
	...
	function goodsDetail(){
		...
		if(len>1){
			goods_id=h_goodsId[arrIdx].value;
		}else{
			goods_id=h_goodsId.value;
		}
		...
		formObj.method="get";
		formObj.action="${contextPath}/goods/goodsDetail.do?goodsId="+goods_id;
		formObj.submit();
...
<div id="sticky" >
		...
		<ul>
			...
			<form name="frm_sticky"  >
				...
				<a href="javascript:goodsDetail();">
					<img width="75" height="95" id="imgSticky"
	...
	<div>
		<c:choose>
			...
			<c:otherwise>
				<h5><a  href='javascript:fn_show_previous_goods();'> 이전 </a> &nbsp;  <span id="curGoodsNum">1</span>/${quickGoodsListNum}  &nbsp; <a href='javascript:fn_show_next_goods();'> 다음 </a> </h5>
			</c:otherwise>
		</c:choose>
	</div>
</div>
